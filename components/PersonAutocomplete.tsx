...
export default function PersonAutocomplete({ person, entries, kind, editId, onChange, onEnter }: Props) {
  const { T } = useApp();
  ...
  const history = useMemo(() => {
    ...
    [...entries]
      .filter(e => e.person && e.person.trim() && e.id !== editId)
      .sort((a, b) => b.date.localeCompare(a.date))
      .forEach(e => {
        const key = e.person.trim().toLowerCase();
        if (!seen.has(key)) seen.set(key, e.person.trim());
      });
    return [...seen.values()];
  }, [entries, editId]);
  ...
  const suggestions = useMemo(() => {
    ...
    return history.filter(p => p.toLowerCase().includes(q)).slice(0, 5);
  }, [person, history]);

  const showDrop = focused && suggestions.length > 0;

  function pick(name: string) { onChange(name); setFocused(false); setCursor(-1); }
  ...
  const placeholder = kind === 'income' ? 'Received from (optional)' : 'Sent to (optional)';
      ...
      {showDrop && (
        <div style={{
          ...
          {suggestions.map((name, i) => {
            ...
            return (
              <div key={name} onMouseDown={() => pick(name)} style={{
              ...
              >
                <User size={13} color={T.muted} strokeWidth={1.8} />
